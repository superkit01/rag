...
export function CitationPopover({ citation }: CitationPopoverProps) {
  return (
    ...
    >
      <p className="text-sm font-semibold text-gray-900">{citation.document_title || 'Unknown Document'}</p>
      ...
      <div className="mt-2 pt-2 border-t border-gray-100">
        <p className="text-xs text-gray-500">
          Score: {Number.isFinite(citation.score) ? citation.score.toFixed(3) : 'N/A'}
        </p>
      </div>
    </motion.div>
  );
