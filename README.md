...
- 文档切割支持 `CHUNKING_STRATEGY=fixed-size` 与 `CHUNKING_STRATEGY=parent-child`：
  ...
  - 可通过 `PARENT_CHUNK_SIZE`、`PARENT_CHUNK_OVERLAP`、`CHILD_CHUNK_SIZE`、`CHILD_CHUNK_OVERLAP` 调整父子切块粒度。切换策略后，建议对历史文档执行重建索引。
